...
assert.equal(codexWindows[0].usedPct, 38);
const codexZero = parseCodexUsage({ rate_limit: { primary_window: { used_percent: 0 }, secondary_window: { used_percent: 1 } } });
assert.equal(codexZero[0].usedPct, 0);
assert.equal(codexZero[0].remainPct, 100);
assert.equal(codexZero[1].usedPct, 1);
assert.equal(codexZero[1].remainPct, 99);
const codexOmitted = parseCodexUsage({ rate_limit: { primary_window: { limit_window_seconds: 18000 }, secondary_window: { used_percent: 2 } } });
assert.equal(codexOmitted[0].usedPct, 0);
assert.equal(codexOmitted[0].remainPct, 100);
assert.equal(codexOmitted[1].remainPct, 98);
const grokWindows = parseGrokUsage({ creditUsagePercent: 22 });
assert.equal(grokWindows[0].id, 'week');
assert.equal(grokWindows[0].remainPct, 78);
assert.equal(grokWindows[0].usedPct, 22);
...
assert.equal(grokFresh[0].usedPct, 0);
const grokZero = parseGrokUsage({ creditUsagePercent: 0 });
assert.equal(grokZero[0].usedPct, 0);
assert.equal(grokZero[0].remainPct, 100);
const grokOne = parseGrokUsage({ creditUsagePercent: 1 });
assert.equal(grokOne[0].usedPct, 1);
assert.equal(grokOne[0].remainPct, 99);
const cursorWindows = parseCursorUsage({ planUsage: { autoPercentUsed: 29, apiPercentUsed: 62 } }, { usagePercent: 12 });
assert.deepEqual(cursorWindows.map(window => window.id), ['auto', 'api', 'bot']);
assert.equal(cursorWindows[0].usedPct, 29);
assert.equal(cursorWindows[0].remainPct, 71);
assert.equal(cursorWindows[1].usedPct, 62);
assert.equal(cursorWindows[2].label, 'Grok Bot 周额度');
assert.equal(cursorWindows[2].remainPct, 88);
const cursorZero = parseCursorUsage({ planUsage: { autoPercentUsed: 0, apiPercentUsed: 1 } }, { usagePercent: 0 });
assert.equal(cursorZero[0].usedPct, 0);
assert.equal(cursorZero[0].remainPct, 100);
assert.equal(cursorZero[1].usedPct, 1);
assert.equal(cursorZero[1].remainPct, 99);
assert.equal(cursorZero[2].usedPct, 0);
assert.equal(cursorZero[2].remainPct, 100);
